...
export function DiffTool() {
  const [left, setLeft] = useLocalStorageState<string>("koalatools_diff_left", "");
  const [right, setRight] = useLocalStorageState<string>("koalatools_diff_right", "");
    ...
    <div className="space-y-4 min-h-0 flex flex-col">
      ...
      {hasInput && (
        <div className="flex-1 min-h-0 flex flex-col animate-in fade-in duration-200">
          ...
          <div className="overflow-auto border border-border rounded-lg font-mono text-xs leading-6 max-h-96">
            {diff.map((line, idx) => (
              <div
                key={idx}
                className={`flex ${
              ...
              >
                ...
                <span
                  className={`px-2 py-0.5 flex-1 whitespace-pre ${
                    line.type === "added"
