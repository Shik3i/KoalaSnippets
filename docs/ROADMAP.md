...
### Comprehensive Keyboard Navigation & Accessibility (A11y)

**Status**: Planned  
**Description**: Make the entire application 100% accessible and keyboard-navigable. Power users and users with assistive technologies should be able to navigate lists, search, view, edit, and manage snippets seamlessly using only their keyboard (`Tab`, `Enter`, `Space`, `Arrow Keys`, and custom hotkeys).

**Example Workflow**:
- A user hits `Cmd/Ctrl + K` to focus the search bar.
- They type a search term, then use the `Up` and `Down` arrow keys to navigate the search results dropdown.
- Pressing `Enter` opens the selected snippet.
- Pressing `E` opens the edit view, `Esc` cancels out of modals, and `Ctrl + S` saves changes.
- All focused elements show a beautifully styled, high-contrast glow ring that fits our dark/glassmorphic design language.

**Technical Implementation Details**:
- **Global Key Event Listener**: Implement a lightweight global hotkey coordinator (e.g., using `react-hotkeys-hook` or native `keydown` event listeners in a custom React hook) to manage shortcuts.
- **Focus Indicator System**: Add global CSS focus-visible styles (e.g., modern `:focus-visible { outline: 2px solid var(--accent-glow); outline-offset: 2px; }`) to ensure high contrast without breaking the premium aesthetic.
- **WAI-ARIA Compliance**: Ensure all interactive elements (modals, dropdowns, tab components) use standard keyboard patterns (like arrow keys for tab lists, `Home`/`End`, etc.) and possess semantic markup with appropriate `aria-*` tags.
- **Skip Link**: Add a hidden "Skip to Main Content" link at the top of the DOM structure for assistive devices.
...
**Status**: Planned  
**Description**: Prevent accidental duplicate snippets and keep the library clean. When a user pastes or writes code for a new snippet, the application checks (against their own existing snippets, using normalized/whitespace-trimmed code comparison) if an identical snippet already exists and prompts them with a helpful warning modal.

**Example Workflow**:
- A user starts creating a new snippet and pastes a utility function they already saved last month as *"API Helper"*.
- Upon attempting to save, a beautiful glassmorphic confirmation modal pops up:  
  *“Duplicate Detected: You already have a snippet named 'API Helper' with the exact same code. Do you want to view it, or create this new one anyway?”*
- The user can choose to view the existing snippet (canceling the creation) or bypass the warning and proceed with creating a duplicate.

**Technical Implementation Details**:
- **Backend API**: Add a check endpoint or Server Action `checkForDuplicateSnippet(code)` that hashes or queries normalized (whitespace-trimmed) code fields of the user's active snippets.
- **Frontend UI**:
  - Integrate the duplicate check into the snippet creation form validation.
  - Create a modern, accessible confirmation dialog/modal allowing choices: "View Existing", "Create Anyway", "Cancel".
- **Estimated Effort**: ~150 lines of code.

---

### Code Line Statistics & Reading Time Estimator

**Status**: Planned  
**Description**: Display code statistics—specifically the line count, estimated reading time, and raw file size—in the snippet detail headers and on dashboard SnippetCards. This minor visual addition provides helpful context at a glance, allowing users to estimate the complexity of a snippet before opening or copying it.

**Example Workflow**:
- A user browses their dashboard.
- On a snippet card labeled *"Django Middleware Utilities"*, they instantly see metadata badges:  
  `🐍 Python` • `142 lines` • `~2 min read` • `4.8 KB`.
- Clicking into the snippet details displays the same rich metadata in the header, matching the glassmorphic styling.

**Technical Implementation Details**:
- **Utility Logic**: Write a simple utility function `calculateSnippetStats(code)` that:
  - Counts the number of lines by splitting on newline characters (`\n`).
  - Estimates reading time based on an average words-per-minute (WPM) speed adjusted for code density, or a lines-per-minute reading scale (e.g., 60 lines/minute).
  - Calculates the raw size in bytes/kilobytes using the string's byte size.
- **Frontend Integration**:
  - Update `SnippetCard` to calculate and render these stats inline with minimal UI footprint.
  - Update the `DetailView` header to render the stats in a clean, secondary-text metadata row.
...
**Status**: Planned  
**Description**: Replace traditional, clunky page-by-page pagination with fluid Infinite Scroll on dashboards and public lists. Instead of basic OFFSET/LIMIT queries that degrade in database performance over large datasets, implement a robust cursor-based pagination model using unique database IDs or timestamps.

**Example Workflow**:
- A power user with 500+ snippets scrolls down their dashboard.
- Instead of clicking "Next Page", the next 20 snippets load seamlessly as they approach the bottom of the screen.
- The URL optionally maintains state via query parameters (e.g., `?cursor=abc123xyz`), making direct linking or page refreshes robust.

**Technical Implementation Details**:
- **Database & API**: 
  - Replace `OFFSET` with cursor-based retrieval (e.g., `WHERE created_at < cursor ORDER BY created_at DESC LIMIT 20`).
  - Update Server Actions or endpoints to accept and return a `nextCursor` value.
- **Frontend Logic**:
  - Implement a highly reusable custom React hook `useIntersectionObserver` or utilize modern browser Intersection Observer APIs.
  - Manage snippet lists dynamically via client-side state, appending new results as the bottom placeholder becomes visible.
  - Implement loading and end-of-list placeholders with smooth animations.
- **Estimated Effort**: ~400 lines of code.
...
**Status**: Planned  
**Description**: Isolate runtime errors and enhance overall application stability by introducing scoped, granular React Error Boundaries around major feature zones (Sidebar, Command Palette, Snippet Grid, Admin Panel). Instead of a single error crashing the entire viewport (global `error.tsx`), only the failing component displays a graceful local fallback UI while the rest of the application remains fully interactive.

**Example Workflow**:
- An edge-case syntax highlighting error or malformed payload occurs inside a newly created snippet card.
- Instead of the entire dashboard rendering a blank error screen, only that specific snippet card displays a beautiful, compact fallback box: *“⚠️ Failed to render snippet content”*.
- The sidebar, search bar, and other snippet cards continue to function flawlessly.

**Technical Implementation Details**:
- **Component Architecture**:
  - Implement a highly reusable `<SafeZone />` React Error Boundary component utilizing `componentDidCatch` or `react-error-boundary`.
  - Equip the boundary with structured logging (e.g., reporting error stacks to our internal log system or an external telemetry tool).
- **Frontend Refactoring**:
  - Wrap major high-risk sections (e.g., individual `SnippetCard` nodes, `CommandPalette`, `SidebarNavigation`, and complex Markdown renderers) inside targeted boundaries.
  - Design custom, glassmorphic localized fallback components tailored to match the parent layout proportions (avoiding layout shifts).
- **Estimated Effort**: ~200 lines of code.

---

### Shared Schema Validation (Client-Side & Server-Side Zod)

**Status**: Planned  
**Description**: Optimize form submittals and reduce server overhead by reusing backend Zod validation schemas (`validations.ts`) directly on the client side (e.g., in combination with React hooks like `useActionState` or form-level pre-submit validation). This prevents unnecessary roundtrips to the server for basic validation errors and provides instant, fluid visual feedback to users.

**Example Workflow**:
- A user attempts to create a new snippet but accidentally leaves the title field blank.
- The moment they focus out of the input or click "Save", a red glowing border instantly surrounds the field with a clear inline message: *“⚠️ Title is required”*.
- No API request or Server Action is triggered, saving server cycles and ensuring zero latency for the user.

**Technical Implementation Details**:
- **Schema Sharing**:
  - Centralize validation logic by exporting reusable Zod schemas from a shared `lib/validations.ts` file accessible by both server action endpoints and client-side components.
- **Client Form Integration**:
  - Implement a highly reusable form validation helper or custom hooks (e.g., using `react-hook-form` with a Zod resolver, or a custom lightweight input validation state hook).
  - Create a modern, accessible `<ValidationError />` inline component that supports micro-animations when displaying error alerts.
...
**Status**: Planned  
**Description**: Keep search and list queries sub-millisecond fast even as the user's snippet repository scales. Introduce optimized database indexes on frequently queried, ordered, and filtered columns (such as `snippets.title`, `snippets.language_id`, `snippets.created_at`, and `tags.name`), including compound/composite indexes for complex search operations.

**Example Workflow**:
- A developer searches their repository of 1,000+ snippets for *"React"* while filtering specifically for the language *"TypeScript"*.
- Rather than performing a slow sequential table scan, the database engine utilizes a combined composite index to instantly narrow down and resolve the records.
- Query execution times drop dramatically from 50ms to less than 5ms.

**Technical Implementation Details**:
- **Query Profiling & Analysis**:
  - Run SQL `EXPLAIN QUERY PLAN` on our primary search, filter, and dashboard retrieval statements to inspect execution paths and spot scanning bottlenecks.
- **Database Schema Migration**:
  - Draft a new migration executing clean `CREATE INDEX` statements (e.g., compound index on `(language_id, created_at)` or full-text index expressions).
- **Estimated Effort**: ~50 lines of code.

---

### Comprehensive Error Diagnostics & Crash Reporting

**Status**: Planned  
**Description**: Complement visual error isolation with a robust, production-grade error diagnostics and crash reporting system. When an error boundary catches a runtime failure, it will log the complete stack trace, user context, and affected snippet/route context to a dedicated server-side database. Simultaneously, the user will be presented with a beautiful, friendly error state showing diagnostic data and a direct "Report Issue / Copy details" button.

**Example Workflow**:
- An unexpected error occurs while rendering a complex markdown snippet.
- The local error boundary catches the exception and automatically fires an anonymous payload containing stack diagnostics, user/browser session data, and the snippet ID to the server.
- The user sees a polished glassmorphic error panel: *“Something went wrong rendering this section”* along with a **📋 Copy Diagnostics** button and a **Report Issue** option.
- The administrator can immediately inspect the logged error trace in their admin dashboard to diagnose the issue.

**Technical Implementation Details**:
- **Database Schema**:
  - Add a `crash_reports` database table containing columns for `id`, `error_message`, `stack_trace`, `user_id` (nullable), `route`, `metadata` (JSON), and `created_at`.
- **API Endpoint**:
  - Create a Server Action or secure API route `POST /api/reports/crash` to receive and store client-side crash telemetry.
- **Frontend Diagnostics UI**:
  - Extend the `<ErrorBoundary />` / `<SafeZone />` fallback component to extract and format error information.
  - Implement a copy-to-clipboard function for the technical diagnostics payload to help users paste error data directly into GitHub issues or support forms.
- **Estimated Effort**: ~250 lines of code.
